package github.FinalProject.alphanotes.fragment.template;
            ...
            public void run() {
                try {
                    if (categoryId == DatabaseModel.NEW_MODEL_ID) {
                        ...
                        List<Note> checkListItem = Note.getCheckListNote(categoryId);
                        items = (ArrayList<T>) Note.all(categoryId);
                        if (checkListItem != null && !checkListItem.isEmpty()) {
                            Note note = checkListItem.get(0);
                            items.add((T) note);
                        }
                    }
