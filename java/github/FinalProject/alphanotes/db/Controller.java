package github.FinalProject.alphanotes.db;
...
public class Controller {
    ...
    public void readBackup(JSONArray json) throws Exception {
        ...
        try {
            ...
            for (int i = 0; i < length; i++) {
                ...
                values.put(OpenHelper.COLUMN_LOCK, item.getInt(OpenHelper.COLUMN_LOCK));
        ...
        try {
            ...
            if (c != null) {
                ...
                while (c.moveToNext()) {
                    ...
                    item.put(OpenHelper.COLUMN_LOCK, c.getInt(c.getColumnIndex(OpenHelper.COLUMN_LOCK)));
     ...
     * @return a list of notes or categories
     */
    public <T extends DatabaseModel> ArrayList<T> findCheckList(Class<T> cls, String[] columns, String where, String[] whereParams, int sortId) {
        ArrayList<T> items = new ArrayList<>();
    ...
    public void addCategoryCounter(long categoryId, int amount) {
        ...
        try {
            Cursor c = db.rawQuery(
                    String.format("UPDATE %s SET %s = %s + ? WHERE %s = ?", OpenHelper.TABLE_NOTES, OpenHelper.COLUMN_COUNTER, OpenHelper.COLUMN_COUNTER, OpenHelper.COLUMN_ID),
                    new String[]{
        ...
        try {
            if (note.id > DatabaseModel.NEW_MODEL_ID) {
                ...
                if (note instanceof Note) {
                    ...
                    Cursor c = db.rawQuery(
                            String.format("UPDATE %s SET %s = %s + 1 WHERE %s = ?", OpenHelper.TABLE_NOTES, OpenHelper.COLUMN_COUNTER, OpenHelper.COLUMN_COUNTER, OpenHelper.COLUMN_ID),
                            new String[]{
